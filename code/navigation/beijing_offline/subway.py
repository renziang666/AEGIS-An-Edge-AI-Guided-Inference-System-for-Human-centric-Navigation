...
class Station:
	...
	def info(self):
		print('您目前位于 %s %s 站' % (self.line[0], self.name))
...
class Path:
	...
	def go(self):
		...
		for i in range(1, len(self.path)):
			...
			if not self.in_line(next_station.name):
				new_line_name = list(set(self.location.line) & set(next_station.line))[0]
				print('乘坐%s线%d站后，在%s从换乘到%s！' % (self.line_name,count_station,self.location.name, new_line_name))
				self.line_name = new_line_name
				count_station = 0
			# print('    %s 站 -----> %s 站' % (self.location.name, next_station.name))
			self.location = next_station
		print('乘坐%s线%d站后，您将会到达位于%s的%s站！全长%s站！' % (self.line_name, count_station+1, self.line_name, self.location.name, len(self.path)-1))
