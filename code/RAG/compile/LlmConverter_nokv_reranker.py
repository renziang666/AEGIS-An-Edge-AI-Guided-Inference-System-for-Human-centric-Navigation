...
class LlmConverter(BaseConverter):
    ...
    def __init__(self, args, config):
        ...
        self.lmhead_with_topk = False #without lmhead
        self.export_kv_cache = False # without kv
        self.export_embedding_cache = False # without_embedding_cache
        self.chip = args.chip
    ...
    def gen_block_mlir(self, idx: int):
        ...
        # ---- 新增本地开关，默认读 self.export_kv_cache；若无此属性则回退 True 以兼容原逻辑
        export_kv = getattr(self, "export_kv_cache", True)
        ...
        def gen_mlp(mlir_gen, input_shape, in_op):
            ...
            gate_op = self.linear(mlir_gen, mlp_gate, new_op,
                                ...
                                [1, length, self.intermediate_size])
            act_op = self.activate(mlir_gen, gate_op, self.hidden_act, mlp_gate)
            up_op = self.linear(mlir_gen, mlp_up, new_op,
                                [self.hidden_size, self.intermediate_size],
                                [1, length, self.intermediate_size])
            new_op = top.MulOp(mlir_gen.get_tensor_type([1, length, self.intermediate_size]),
                            [act_op, up_op],
        ...
        def gen_block():
            ...
            # ---- 关键：根据 export_kv 切换输出签名（是否包含 kv）
            out_shapes = [input_shape] + ([kv_shape, kv_shape] if export_kv else [])
            ...
            # rotary pos
            q_op, k_op = self.apply_rotary_pos(block_mlir, in1_op, q_op, k_op, rotary_cos, rotary_sin)

            # attention
            fa_op = top.FAttentionOp(T([1, input_len, q_dim]),
                                    q_op, k_op, v_op, in2_op, block_mlir.none_op,
            ...
            # ---- 关键：根据 export_kv 决定是否把 k/v 作为返回
            if export_kv:
                block_mlir.create_return_op([new_op, k_op, v_op])
            else:
        ...
        def gen_block_with_kv():
            # 同理：只有 export_kv=True 且 self.use_block_with_kv=True 时才会被调用
            name = f"block_{idx}"
            input_len = self.max_input_length
            input_shape = [1, input_len, self.hidden_size]
            id_shape = list(self.position_shape)
        ...
        # ---- 关键：生成哪些子图
        if self.use_block_with_kv and export_kv:
            gen_block_with_kv()
        else:
            gen_block()
    ...
    def compile_all(self):
        ...
        #     self.compile_sample_head() # 后处理逻辑暂时numpy手动完成，不要开这个
